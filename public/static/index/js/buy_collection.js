$('#J_tab_type li').click(function () {
    ...
    $('#J_select_collection option').addClass('hidden');
    show_option.removeClass('hidden');
    $('#J_select_collection').val(show_option.attr('value'));
    $('#J_tip_price').html(show_option.data('tip'));
});

$('#J_select_collection').change(function(){
    $('#J_select_collection option').each(function () {
...
$('#J_form_submit').click(function(){
    ...
    $("#J_collection_form").ajaxSubmit({
        url: '/ajax/index.order/buyCollection',
        type: 'post',
        dataType: 'json',
